...
export function ReportsPanelModal({ isOpen, onClose }: ReportsPanelModalProps) {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const filename = `relatorio-helpdesk-${dateStr}.xlsx`;
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.text('Relatório Help Desk', pageWidth / 2, 20, { align: 'center' });
      ...
      // Gerar nome do arquivo
      const dateStr = new Date().toLocaleDateString('pt-BR').replace(/\//g, '-');
      const filename = `relatorio-helpdesk-${dateStr}.pdf`;
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto" aria-describedby="reports-description">
        <DialogHeader>
          ...
          <div id="reports-description" className="sr-only">
            Visualize estatísticas e gere relatórios do sistema de help desk.
          </div>
        </DialogHeader>
