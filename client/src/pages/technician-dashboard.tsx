...
export default function TechnicianDashboard() {
  ...
  const { data: tickets = [], isLoading } = useQuery<TicketWithDetails[]>({
    queryKey: ["/api/tickets", currentUser?.id],
    queryFn: async () => {
      if (!currentUser?.id) return [];
      const response = await fetch(`/api/tickets?technicianId=${currentUser.id}`);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "resolved":
        return <Badge className="badge-resolved">Resolvido</Badge>;
      default:
  ...
  const ResolvedTicketCard = ({ ticket }: { ticket: TicketWithDetails }) => (
    <Card 
      className="hover:shadow-md transition-shadow resolved-ticket-card"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl font-bold">Help Doctum - Painel Técnico</h1>
            </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted-foreground text-sm">Total Abertos</p>
                  <p className="text-2xl font-bold">{stats?.open + stats?.inProgress || 0}</p>
                </div>
